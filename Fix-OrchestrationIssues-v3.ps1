...
Write-Host "CLI ORCHESTRATOR FIX SCRIPT V3" -ForegroundColor Cyan
...
if (Test-Path $monitoringPath) {
    ...
    if ($content -match 'function Process-SignalFile') {
        ...
        # Simple approach: find the function and add a return statement at the beginning
        # This avoids complex regex replacement
        ...
        for ($i = 0; $i -lt $lines.Count; $i++) {
            ...
            if ($line -match 'function\s+Process-SignalFile') {
                $inFunction = $true
                $functionDepth = 0
                $newLines += $line
                ...
                if ($line -match '\{') {
                    $functionDepth = 1
                    # Add our early return on the next line
                    $newLines += '    # DISABLED TO PREVENT DUPLICATE SIGNAL PROCESSING'
                    $newLines += '    Write-Host "[DISABLED] Process-SignalFile in MonitoringLoop - skipping to prevent duplication" -ForegroundColor DarkGray'
                    $newLines += '    return $null'
                    $functionModified = $true
                }
            }
            elseif ($inFunction -and -not $functionModified -and $line -match '\{') {
                $functionDepth = 1
                $newLines += $line
                # Add our early return after the opening brace
                $newLines += '    # DISABLED TO PREVENT DUPLICATE SIGNAL PROCESSING'
                $newLines += '    Write-Host "[DISABLED] Process-SignalFile in MonitoringLoop - skipping to prevent duplication" -ForegroundColor DarkGray'
                $newLines += '    return $null'
                $functionModified = $true
