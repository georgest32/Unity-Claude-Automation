...
# Mock functions for testing when modules not available
function New-ThreadSafeCPG {
    return @{ Type = "MockCPG"; AddNode = { param($name, $type) } }
}
...
function Build-CallGraph {
    param($SourceCode, $Language)
    return @{
        Nodes = @(@{ Name = "Function1" }, @{ Name = "Function2" })
        Edges = @(@{ Source = "Function1"; Target = "Function2" })
    }
}
...
function Trace-DataFlow {
    param($SourceCode, $Language)
    return @{
        DefUseChains = @(@{ Variable = "test"; Definitions = @(1); Uses = @(2, 3) })
    }
}

function Invoke-TaintAnalysis {
    param($SourceCode, $Language)
    return @{
        TaintedPaths = @(@{ Variable = "userInput"; Path = @("userInput", "sqlQuery") })
    }
}
...
Describe "Enhanced Documentation System - CPG Components" -Tag "CPG", "Core" {
        ...
        BeforeDiscovery {
            Write-Host "[BeforeDiscovery-CPG] Ensuring CPG module variables available in run phase..." -ForegroundColor Blue
            # Re-initialize to ensure availability in run phase
            $script:CPGModulesAvailable = @{
                'CPG-ThreadSafeOperations' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-CPG\Core\CPG-ThreadSafeOperations.psm1")
                'CPG-Unified' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-CPG\Core\CPG-Unified.psm1")
                'CPG-CallGraphBuilder' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-CPG\Core\CPG-CallGraphBuilder.psm1")
            ...
            Write-Host "[BeforeDiscovery-CPG] CPG variables re-initialized for run phase" -ForegroundColor Blue
        }
...
Describe "Enhanced Documentation System - LLM Integration" -Tag "LLM", "Integration" {
        ...
        BeforeDiscovery {
            Write-Host "[BeforeDiscovery-LLM] Ensuring LLM module variables available in run phase..." -ForegroundColor Blue
            # Re-initialize to ensure availability in run phase
            $script:LLMModulesAvailable = @{
                'LLM-PromptTemplates' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-LLM\Core\LLM-PromptTemplates.psm1")
                'LLM-ResponseCache' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-LLM\Core\LLM-ResponseCache.psm1")
            }
            Write-Host "[BeforeDiscovery-LLM] LLM variables re-initialized for run phase" -ForegroundColor Blue
        }
...
Describe "Enhanced Documentation System - Templates & Automation" -Tag "Templates", "Automation" {
        ...
        BeforeDiscovery {
            Write-Host "[BeforeDiscovery-Templates] Ensuring Template module variables available in run phase..." -ForegroundColor Blue
            # Re-initialize to ensure availability in run phase
            $script:TemplateModulesAvailable = @{
                'Templates-PerLanguage' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-Enhanced-DocumentationGenerators\Core\Templates-PerLanguage.psm1")
                'AutoGenerationTriggers' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-Enhanced-DocumentationGenerators\Core\AutoGenerationTriggers.psm1")
            }
            Write-Host "[BeforeDiscovery-Templates] Template variables re-initialized for run phase" -ForegroundColor Blue
        }
...
Describe "Enhanced Documentation System - Performance" -Tag "Performance", "Benchmarking" {
        ...
        BeforeDiscovery {
            Write-Host "[BeforeDiscovery-Performance] Ensuring Performance module variables available in run phase..." -ForegroundColor Blue
            # Re-initialize to ensure availability in run phase
            $script:PerfModulesAvailable = @{
                'Performance-Cache' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-CPG\Core\Performance-Cache.psm1")
                'Performance-IncrementalUpdates' = (Test-Path "$PSScriptRoot\Modules\Unity-Claude-CPG\Core\Performance-IncrementalUpdates.psm1")
            ...
            Write-Host "[BeforeDiscovery-Performance] Performance variables re-initialized for run phase" -ForegroundColor Blue
        }
