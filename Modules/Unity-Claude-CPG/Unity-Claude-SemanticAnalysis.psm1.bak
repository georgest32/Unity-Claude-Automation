...
function Find-DesignPatterns {
    ...
    begin {
        ...
        $nodeCount = (Unity-Claude-CPG\Get-CPGNode -Graph $Graph).Count
        $edgeCount = (Unity-Claude-CPG\Get-CPGEdge -Graph $Graph).Count
        $cacheKey = "PAT::$nodeCount::$edgeCount::" + ($PatternTypes -join ',') + "::$MinConfidence"
    ...
    process {
        ...
        return ,$patterns  # Use comma operator to ensure array is returned
    }
}
...
function Get-CodePurpose {
    ...
    begin {
        ...
        $cacheKey = "PURP::$nodeCount::$edgeCount::$nodeIdKey"
    ...
    process {
        try {
            ...
            foreach ($node in $nodes) {
                Write-Verbose "Analyzing purpose of: $($node.Name)"
                
                # First try fast name-based classification
                $quickPurpose = Resolve-PurposeFromName -Name $node.Name
                if ($quickPurpose) {
        ...
        return ,$purposeResults  # Use comma operator to ensure array is returned
    }
}
        ...
        try {
            ...
            foreach ($module in $modules) {
                ...
                $cohesionResult = @{
                    ...
                    CHM = [Math]::Round((Clamp01 -Value $chm), 4)
                    CHD = [Math]::Round((Clamp01 -Value $chd), 4)
                    OverallCohesion = [Math]::Round((Clamp01 -Value $overallCohesion), 4)
    ...
    foreach ($analysisType in $ReportData.Analysis.Keys) {
        ...
        $html += @"
                <div class="analysis-item">
                    ...
                    <p><strong>Items Analyzed:</strong> $(if ($analysisData.Count) { $analysisData.Count } else { 'N/A' })</p>
                </div>
"@
    }
