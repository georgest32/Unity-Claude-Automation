# Unity-Claude-Cache-Fixed.psm1
# Redis-like in-memory cache implementation for high-performance documentation system
# Fixed version with proper timer handling and return values
...
class CacheManager {
    ...
    # Set cache item with optional TTL and priority - FIXED to return boolean properly
    ...
    # Manual cleanup expired items (no timer)
    [void]CleanupExpired() {
    ...
    [bool]NeedsCleanup() {
        return ([datetime]::Now - $this.LastCleanup).TotalSeconds -gt 60
    }
...
function Set-CacheItem {
    ...
    if ($TTLSeconds -gt 0) {
        $result = $CacheManager.Set($Key, $Value, $TTLSeconds, $Priority)
    }
    else {
        $result = $CacheManager.Set($Key, $Value, $null, $Priority)
    }
    
    # Suppress the boolean output
    Write-Output $result -NoEnumerate | Out-Null
    return $result
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDk927KR3f3kCec
# 0qV0EB+ug8jr1+/QxrUSPXDwrlyXbaCCAzAwggMsMIICFKADAgECAhB1HRbZIqgr
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIBcDma4geyccjwHbFtbY1Sup
# +h5+Gmq7vvWAxz9q0JG7MA0GCSqGSIb3DQEBAQUABIIBAElO2Jpv0nm3VEa2VFY2
# eSl5zF+GT6HsRpAwkNYKD6EyWpkVhZa4h44IDOj1BfMOd9I/Ext+Xfti2THCC7Cc
# jnAH10JEjIONIg/G0+MQqclaWVKa+vpGSxIw7BaE847syLXojTaBG+b5NxsTY+Tz
# NZjsVnDl4ixm++N/3nRvirOYwio8fS+nfirdA8C8hMeLtvrM5Yq6DHKPUef6Tpyz
# 7+a8VieWiZZULewo++Roh5xfASGNQMdK92nOJ+srZKZMPpT+VjWnfHbhg4LPyiwU
# ZoiU7WzqEZLK/fn0d5mdDlpeYMGKSdB0jhHpxvlAGNIfL6BwVzlhdCrH8V+3jToM
# uvk=
