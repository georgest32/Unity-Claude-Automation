...
// Development mode hot reload - DISABLED to avoid port conflicts
console.log(`[DEV] Livereload disabled to avoid port conflicts - restart server manually for changes`);
  ...
  try {
    ...
    try {
      if (fs.existsSync(dataPath)) {
        ...
        console.log(`[API] Loaded real data with ${realData.nodes.length} nodes and ${realData.links.length} edges`);
      } else {
    ...
    res.json(realData);
    console.log(`[API] Served real data with ${realData.nodes.length} nodes and ${realData.links.length} edges`);
  } catch (error) {
