...
try {
    ...
    foreach ($cmd in $safeCommands) {
        $safety = Test-CommandSafety -CommandName $cmd -Parameters @("C:\test.txt")
        $color = if ($safety.IsSafe) { "Green" } else { "Red" }
        Write-Host "  Safe command $cmd`: $($safety.IsSafe) (Risk: $($safety.RiskLevel))" -ForegroundColor $color
    }
    ...
    foreach ($cmd in $dangerousCommands) {
        ...
        $color = if (-not $safety.IsSafe) { "Green" } else { "Red" }
        Write-Host "  Dangerous command $cmd`: Blocked=$(-not $safety.IsSafe) (Risk: $($safety.RiskLevel))" -ForegroundColor $color
    }
}
catch {
...
try {
    ...
    foreach ($value in $testValues) {
        ...
        Write-Host "  '$value' -> '$sanitized' (Sanitized: $wasSanitized)" -ForegroundColor $color
    }
}
catch {
...
try {
    ...
    $result = Invoke-SafeConstrainedCommand -CommandName "Get-Date" -Parameters @{ Format = "yyyy-MM-dd HH:mm:ss" } -TimeoutMs 10000
    ...
    $blockedResult = Invoke-SafeConstrainedCommand -CommandName "Invoke-Expression" -Parameters @{ Command = "Get-Process" }
...
try {
    $dryRunResult = Invoke-SafeConstrainedCommand -CommandName "Get-Content" -Parameters @{ Path = "C:\UnityProjects\Sound-and-Shoal\test.txt" } -DryRun
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUG6z+ZB+C0aCo43nRkKAC5mQx
# spKgggMwMIIDLDCCAhSgAwIBAgIQdR0W2SKoK5VE8JId4ZxrRTANBgkqhkiG9w0B
...
# CQQxFgQUCA6y8/RHneAf2+8eWEUbVwULfY4wDQYJKoZIhvcNAQEBBQAEggEAc3QY
# H3z13X9A5Fwc1aYtAZHCCM18MEVBbdn8t7zH6wX189yub9CwH3zRfbdQi0ytJEPP
# KeCHLR91l5qSzLmVa8UfPVF0u8S39/DHtvQV2AcP7WCaqJ1Otssl2x042W05cw5W
# n/2+Z8qEVI9YXHLgkS4QIIvn60RDw8H6dcT+bfUnV86Oe0Jr69jIgw41bLci2a44
# TWbJcjdCUPYnXzeUWstfA44JRUH7x+vZbCB5n5brSMvxVsurrH5vB1DJ79OQlznE
# 0Bm84IOSB7dOH8tS+AZ7O5WjdfH6IrqK4KgJDB2dKjxLeaZXIpsXw3AAcFHI9OpZ
# BC7mBJFXs99dz8RI3g==
